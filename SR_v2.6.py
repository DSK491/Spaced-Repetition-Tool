...
1. Improved performance of 'Update Statuses'.
2. 'Update Statuses' is more verbose.
3. 'View/Log Data' now allows logging data multiple times without returning to main menu.
...
def Update_Data():
    ...
    for table in tablenames:
        ...
        while num <= 3:
            ...
            db.execute(f"UPDATE \"{table}\" SET Status{num} = -1 WHERE (Date{num} = \"{presdate}\" AND Status{num} NOT IN (1, -1))")
            updt += db.rowcount
            # print(db.rowcount)
            ...
            for posdate in Datecur:
                ...
                if POSDATE < PRESDATE:
                    ...
                    db.execute(f"UPDATE \"{table}\" SET Status{num} = -2 WHERE (Date{num} = \"{posdate[0]}\" AND Status{num} NOT IN (1, -2, -3))")
                    updt += db.rowcount
                    # print(f"past: {db.rowcount}")
                    if POSDATE + datetime.timedelta(days = 7) < PRESDATE:
                        #print('very overdue')
                        # Setting status for reminders 'overdue - won't remind'
                        db.execute(f"UPDATE \"{table}\" SET Status{num} = -3 WHERE Date{num} = \"{posdate[0]}\" AND (Status{num} NOT IN (1, -2, -3))")
                        updt += db.rowcount
                ...
                if POSDATE > PRESDATE:
                    ...
                    db.execute(f"UPDATE \"{table}\" SET Status{num} = 0 WHERE Date{num} = \"{posdate[0]}\" AND (Status{num} NOT IN (-1, 0))")
                    updt += db.rowcount
...
def View_Data():
    ...
    while repcon == 1:
        ...
        if len(incomlis) != 0:
            repcon = input("\nPress Spacebar and Enter to log data again, or Press Enter to return to Main Menu...")
        if len(incomlis) == 0:
